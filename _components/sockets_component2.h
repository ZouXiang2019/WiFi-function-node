...
//#define CONFIG_PACKET_RATE      CONFIG_PACKET_RATE
char *data2 = "1\n";

// void set_data2(char *data_set)
// {
//     strcpy(data2, data_set);
// }


void socket_transmitter_sta_loop2() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.2";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data2, strlen(data2), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data2)) {
                vTaskDelay(1);
                continue;
            }
