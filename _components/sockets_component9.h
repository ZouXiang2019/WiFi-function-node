...
// void set_data9(char *data_set)
// {
//     strcpy(data9, data_set);
// }


void socket_transmitter_sta_loop9() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.9";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data9, strlen(data9), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data9)) {
                vTaskDelay(1);
                continue;
            }
