...
// void set_data3(char *data_set)
// {
//     strcpy(data3, data_set);
// }


void socket_transmitter_sta_loop3() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.3";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data3, strlen(data3), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data3)) {
                vTaskDelay(1);
                continue;
            }
