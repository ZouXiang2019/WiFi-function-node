...
// void set_data5(char *data_set)
// {
//     strcpy(data5, data_set);
// }


void socket_transmitter_sta_loop5() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.5";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data5, strlen(data5), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data5)) {
                vTaskDelay(1);
                continue;
            }
