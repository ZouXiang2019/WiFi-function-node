...
void _wifi_csi_cb(void *ctx, wifi_csi_info_t *data) {
    ...
    strcat(output, "CSI_DATA,");
    strcat(output, project_type);
    strcat(output, ",");
    strcat(output, mac);
    strcat(output, ",CSI_DATA,");
    char temp[1000];
    sprintf(temp, "%d,", d.rx_ctrl.rssi);
    strcat(ooutput, temp);
...
void csi_init(char *type) {
    ...
    configuration_csi.channel_filter_en = 0;/**< enable to turn on channel filter to smooth adjacent sub-carrier. Disable it to keep independence of adjacent sub-carrier. Default enabled */
