...
// void set_data6(char *data_set)
// {
//     strcpy(data6, data_set);
// }


void socket_transmitter_sta_loop6() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.6";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data6, strlen(data6), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data6)) {
                vTaskDelay(1);
                continue;
            }
