...
// void set_data4(char *data_set)
// {
//     strcpy(data4, data_set);
// }


void socket_transmitter_sta_loop4() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.4";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data4, strlen(data4), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data4)) {
                vTaskDelay(1);
                continue;
            }
