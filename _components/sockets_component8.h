...
// void set_data8(char *data_set)
// {
//     strcpy(data8, data_set);
// }


void socket_transmitter_sta_loop8() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.8";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data8, strlen(data8), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data8)) {
                vTaskDelay(1);
                continue;
            }
