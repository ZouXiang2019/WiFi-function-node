...
// void set_data7(char *data_set)
// {
//     strcpy(data7, data_set);
// }


void socket_transmitter_sta_loop7() {
    int socket_fd = -1;
    while (1) {
        close(socket_fd);
        char *ip = "192.168.4.7";
        ...
        while (1) {
            ...
            if (sendto(socket_fd, &data7, strlen(data7), 0, (const struct sockaddr *) &caddr, sizeof(caddr)) !=
                strlen(data7)) {
                vTaskDelay(1);
                continue;
            }
